...
import { getNotificationKeys, getSettingsKeys } from "../../utils/dynamodb.mjs";
import {
  fetchScraping,
  sampleScraping,
  processMacros,
...
export async function handler(event, rest) {
  ...
  for (let { body = {} } of Records) {
    ...
    const { diaryEntries } = processMacros(response);
    ...
    const comparisonText = generateComparisonText(currentResults, {
      calories,
      carbs,
      protein,
      fat,
    ...
    console.log("Update App Notification");
